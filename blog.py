...
class PostPage(BlogHandler):
    ...
    def get(self, post_id):
        ...
        self.render('full-post.html', post=post,
                    noOfLikes=likes.count(), comments=comments,
                    error=error)
...
class EditComment(BlogHandler):
    ...
    def get(self, post_id, comment_id):
        if self.user:
            ...
            c = db.get(key)
            if c.user_id == self.user.key().id():
                self.render('edit-comment.html', comment=c.comment)
            else:
                self.redirect('/blog/' + post_id
...
class Signup(BlogHandler):
    ...
    def post(self):
        ...
        self.verify = self.request.get('re-enter')
...
class Login(BlogHandler):
    ...
    def get(self):
        self.render('signin.html', error=self.request.get('error'))
